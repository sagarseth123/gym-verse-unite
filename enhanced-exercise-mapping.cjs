...
async function generateExerciseImage(exerciseName) {
  ...
  try {
    ...
    const fileName = `${exerciseName.toLowerCase().replace(/[^a-z0-9]/g, '_')}.png`;
...
async function mapExerciseToImage(exerciseName) {
  ...
  if (generatedImage) {
    console.log(`✅ Generated new image: ${generatedImage}`);
    return `/images/exercise/${generatedImage}`;
  }
...
async function updateExerciseImages() {
  try {
    console.log('🚀 Starting enhanced exercise image mapping...');
    
    // Fetch from category_exercises and use exercise_name
    ...
    for (const exercise of exercises) {
      ...
      let exerciseData = exercise.exercise_data || {};
      console.log(`\n--- Processing: ${exerciseName} ---`);
      
      // Skip if already has a proper image
      if (exerciseData.image_url && !exerciseData.image_url.includes('push_ups.png')) {
        console.log(`⏭️  Already has image: ${exerciseData.image_url}`);
        continue;
      }
      
      const newImageUrl = await mapExerciseToImage(exerciseName);
      
      if (newImageUrl && newImageUrl !== exerciseData.image_url) {
        exerciseData.image_url = newImageUrl;
        // Update database
        ...
        if (updateError) {
          ...
          console.log(`✅ Updated ${exerciseName} with ${newImageUrl}`);
          updatedCount++;
